...
import { TopWorkerCard } from "./components/top-worker-card";
...
export function OperationsDashboard() {
  ...
  return <div className="space-y-8"><PageSection><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{kpis.map((kpi) => <KpiCard key={kpi.label} {...kpi} />)}</div></PageSection>
    <PageSection title="Bosqich qoldiqlari" description="Har bir ishlab chiqarish bosqichida hozir nechta mahsulot turgani">{summary.stageTotals.length > 0 ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{summary.stageTotals.map((stage) => <StageInventoryCard key={stage.stageId} stage={stage} />)}</div> : <EmptyState title="Bosqichlar mavjud emas" description="Faol ishlab chiqarish bosqichlari sozlangandan keyin ular shu yerda ko‘rinadi." />}</PageSection>
    ...
    <PageSection title="Bugungi eng faol ishchilar" description="Smena qabul qiluvchi kiritgan tizim hisoblagan faolliklar">{summary.topWorkers.length > 0 ? <ResponsiveDataList items={summary.topWorkers} getKey={(worker) => `${worker.employeeId}-${worker.stageName}`} renderCard={(worker) => <TopWorkerCard worker={worker} />} ariaLabel="Bugungi eng faol ishchilar" emptyTitle="Bugungi ishchi faolligi yo‘q" emptyDescription="Smena qabul qiluvchi faollik kiritgach, eng faol ishchilar shu yerda ko‘rinadi."><DataTable label="Bugungi eng faol ishchilar"><DataTableHead><DataTableRow><DataTableHeader>Xodim</DataTableHeader><DataTableHeader>Bosqich</DataTableHeader><DataTableHeader>Miqdor</DataTableHeader><DataTableHeader>Hisoblangan summa</DataTableHeader></DataTableRow></DataTableHead><tbody>{summary.topWorkers.map((worker) => <DataTableRow key={`${worker.employeeId}-${worker.stageName}`}><DataTableCell className="font-semibold">{worker.employeeName}</DataTableCell><DataTableCell>{worker.stageName}</DataTableCell><DataTableCell>{formatNumber(Number(worker.quantity))} dona</DataTableCell><DataTableCell className="font-semibold">{formatCurrency(worker.amount)}</DataTableCell></DataTableRow>)}</tbody></DataTable></ResponsiveDataList> : <EmptyState title="Bugungi ishchi faolligi yo‘q" description="Smena qabul qiluvchi faollik kiritgach, eng faol ishchilar shu yerda ko‘rinadi." />}</PageSection>
