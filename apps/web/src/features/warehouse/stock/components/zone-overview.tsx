...
export function ZoneOverview({ zones }: { zones: ZoneSummary[] }) { return <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{zones.map((zone) => <InfoCard key={zone.zoneId} title={formatWarehouseZoneName(zone.zoneName)} action={<StatusBadge tone={statusTone[zone.status]}>{statusLabel[zone.status]}</StatusBadge>}><div className="flex items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-primary/15 text-primary"><MapPin size={18}/></span><div><p className="text-sm text-muted-foreground">{formatWarehouseZoneName(zone.warehouseName)}</p><p className="mt-3 font-semibold">{formatNumber(Number(zone.productQuantity))} dona</p><p className="mt-1 text-xs text-muted-foreground">{zone.productRecordCount} mahsulot · {zone.materialRecordCount} material yozuvi</p></div></div></InfoCard>)}</div>; }
