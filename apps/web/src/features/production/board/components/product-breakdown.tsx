...
import { ProductBreakdownCard } from "./product-breakdown-card";
...
export function ProductBreakdown({ stage, inventory }: { stage: StageTotal; inventory: StageInventory[] }) {
  return (
    <InfoCard
      title={`${stage.stageName} — mahsulot tarkibi`}
      description="Bu bosqichdagi mahsulotlar rang, material va mavsum bo‘yicha"
    >
      <ResponsiveDataList
        items={inventory}
        getKey={(item) => item.id}
        renderCard={(item) => <ProductBreakdownCard item={item} />}
        ariaLabel={`${stage.stageName} mahsulot tarkibi`}
        emptyTitle="Mahsulot mavjud emas"
        emptyDescription="Bu bosqichda hozircha mahsulot yo‘q."
      >
        <DataTable label={`${stage.stageName} mahsulot tarkibi`} className="-mx-5 -mb-5 rounded-t-none border-x-0 border-b-0">
          <DataTableHead>
            <DataTableRow>
          ...
          <tbody>
            {inventory.length > 0 ? (
              ...
              <EmptyTableState colSpan={5} title="Mahsulot mavjud emas" description="Bu bosqichda hozircha mahsulot yo‘q." />
            )}
          </tbody>
        </DataTable>
      </ResponsiveDataList>
