...
import { CorrectionRequestButton } from "@/features/production/board/components/correction-request-panel";
...
export function SupplierDetailsPanel({ debt, purchases, payments, isArchiving, onCreatePurchase, onCreatePayment, onEdit, onArchive }: SupplierDetailsPanelProps) {
  ...
  return <section className="min-w-0 space-y-6 rounded-xl border border-border/70 bg-card/30 p-4 sm:p-5" aria-label={`${debt.supplier.name} ish maydoni`}>
    ...
    <div className="grid gap-3 sm:grid-cols-3"><InfoCard title="Jami xarid"><p className="text-xl font-bold">{formatCurrency(debt.totalPurchases)}</p></InfoCard><InfoCard title="To‘langan"><p className="text-xl font-bold">{formatCurrency(debt.totalPaid)}</p></InfoCard><InfoCard title="Hozirgi qarz"><p className="text-xl font-bold text-amber-500">{formatCurrency(debt.debt)}</p></InfoCard></div>
    <p className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2 text-sm text-muted-foreground">Qarz = bekor qilinmagan xaridlar jami ({formatCurrency(debt.totalPurchases)}) − xaridlarga taqsimlangan to‘lovlar ({formatCurrency(debt.totalPaid)}).</p>
...
function formatDate(value: string): string { return formatDateShort(new Date(value)); }
